...
| ÖPNV-Routing via `/plan` (Abschnitt 5) | 🟡 Beta | eigenes Profil „ÖPNV (β)“ via api.transitous.org: arriveBy nächster Werktag 08:00, `numItineraries=1`, hartes Cap 30 Routen, Concurrency 2, Session-Cache (gerundete Koordinaten, In-Flight-Sharing), `direct`-Fallback wenn Laufen schneller ist; Legs nach Modus gefärbt (Fußwege gestrichelt), Aggregation pro (Linie, Ein-/Ausstieg) statt edge_id. Offen: Zeitwahl-UI, Stop-Pair-genaue Aggregation, ggf. eigene MOTIS-Instanz |
...
| Gebietsanalyse (experimentell, `docs/ANALYSE_MODUS_KONZEPT.md`) | 🟡 Beta | eigener Panel-Tab: Polygon zeichnen → Schulen/Kitas aus schools.pmtiles finden (Node/Way-Doppel dedupliziert) → Fahrten je Einrichtung + Modal Split je Typ editieren → gewichtete Stichproben-Umlegung (Cap `ANALYSIS_MAX_SAMPLE=150` je Einrichtung×Modus, Gewicht = Fahrten/Stichprobe) → Belastungskarte in Fahrten/Tag + GeoJSON-Export (`trips`, `trips_<modus>`, Metadaten). ÖPNV als Fußweg-Zubringer ab zielnächsten Haltestellen — bewusst ohne Transitous (Fair Use). Zensus + Haltestellen werden einmal fürs Gesamtgebiet geladen (je Einrichtung wäre ~5 s × N). Offen: RegioStaR7-Defaults, Auto-Belastung getrennt darstellen |
